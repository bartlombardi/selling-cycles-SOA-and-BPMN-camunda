...
define leggiImpostazioni {
	parseIniFile@IniUtils("fornitore.conf")(configInfo);
...
define inizializzaPorte {
	InterfacciaFornitore.location = location
}
...
init {
	println@Console("#ServizioFornitore")();
	...
	scope(selection) {
		for(i = 0, i < #pezzi, ++i) {
			...
			if(#queryResponse.row > 0) {
				...
				pezziDisponibili[nPezziDisponibili].prezzo = queryResponse.row[0].PREZZO
			}
		}
	};
...
}]{messaggioDaStampare = "[INFO] Invocata verificaDisponibilitaPezzi()()"; log}
...
}]{messaggioDaStampare = "[INFO] - Invocata prenotaPezzi()()"; log }
...
[eliminaPrenotazionePezzi(idOrdine)] {
	...
	messaggioDaStampare = "[INFO] Invocata eliminaPrenotazionePezzi()()"; log
}
...
[eseguiOrdinePezzi(idOrdine)] {
	...
	messaggioDaStampare = "[INFO] Invocata eseguiOrdinePezzi()()"; log
}
